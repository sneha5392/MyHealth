...
@application.route('/sendemail', methods=['GET', 'POST'])
def sendemail():
    ...
    <body>
      ...
      <p>
      Username: """+str(decode(key, username))+"""</br>
      Link: <a href='http://myhealth-env-2.jbpf27us8r.us-west-2.elasticbeanstalk.com/closedashboard1'>"""+str(SENDERNAME)+""" Dashboard's link </a></br>
      Token: """+str(token)+"""
        </p>
    </body>
    </html>
...
@application.route('/closedashboard1', methods=['Get'])
def closedashboard1():
    return render_template("closelogin.html")

@application.route('/closedashboard', methods=['Post'])
def closedashboard():
    username = request.form['Username']
    ...
    t=[]
    for r in response['Items']:
        u.append(decode(key, r['UserName']))
